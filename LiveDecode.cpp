...
int main(int argc, char** argv) {
  ...
  Client mqClient("127.0.0.1", 9000);
  mqClient.use("decoder_result");
  mqClient.watch("decoder_request");
  ...
  while (true) {
    /* ===================== Wait for job ====================== */
    Job job;
    mqClient.reserve(job);
    json jobBody = json::parse(job.body());

    jobId = jobBody["id"];
    audioFilepath = jobBody["audioFile"];
    ...
    if (FLAGS_emission_dir.empty()) {
      ...
      int worldRank = 0;
      int worldSize = 1;
      auto ds = std::make_shared<W2lDirectDataset>(
        audioFilepath,
        dicts,
        lexicon,
        1,
      ...
      // Clear emissions
      emissionSet.emissions.clear();
      emissionSet.wordTargets.clear();
      emissionSet.tokenTargets.clear();
      emissionSet.sampleIds.clear();
      emissionSet.emissionT.clear();
    ...
    auto runDecoder = [&](int tid, int start, int end) {
      try {
        ...
        for (int s = start; s < end; s++) {
          ...
          json decResult;
          decResult["type"] = "decoder_result";
          decResult["audioFile"] = audioFilepath;
          decResult["id"] = jobId;
          decResult["transcript"] = wordPredictionStr;
          LOG(INFO) << decResult.dump();
          mqClient.put(decResult.dump());
