...
namespace {
std::shared_ptr<w2l::augmentation::SoundEffectChain>
createConfiguredSoundEffectChain();
} // namespece
...
int main(int argc, char** argv) {
  ...
  /* ===================== Sound Effects For Augmentation ===================== */
  ...
  std::shared_ptr<w2l::augmentation::SoundEffectChain> trainDsSoundEffect =
      createConfiguredSoundEffectChain();
  std::shared_ptr<w2l::augmentation::SoundEffectChain> validDsSoundEffect =
      createConfiguredSoundEffectChain();

  /* ===================== Create Dataset ===================== */
  auto trainds = createDataset(
      FLAGS_train, dicts, lexicon, FLAGS_batchsize, worldRank, worldSize);
  trainds->augment(trainDsSoundEffect);
  ...
  for (const auto& s : validTagSets) {
    ...
    if (FLAGS_sfx_valid_dataset) {
      validds[s.first]->augment(validDsSoundEffect);
    }
  }
        ...
        while (curBatch < nbatches) {
          ...
          for (auto& batch : *trainset) {
            ...
            if (FLAGS_sfx_start_update >= 0 &&
              ...
              trainDsSoundEffect->enable(true);
              validDsSoundEffect->enable(true);
            }
            ...
            if (FLAGS_reportiters > 0 && curBatch % FLAGS_reportiters == 0) {
              if (validDsSoundEffect) {
                validDsSoundEffect->reset();
              }
              runValAndSaveModel(
...
createConfiguredSoundEffectChain() {
  ...
  std::cout << "soundEffect={" << soundEffectChain->prettyString() << "}"
            << std::endl;
