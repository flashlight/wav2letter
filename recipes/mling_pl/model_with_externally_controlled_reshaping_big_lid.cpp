...
class myModel : public fl::Container {
 public:
  myModel(int64_t nFeature, int64_t nLabel) {
    ...
    for (int trIdx = 0; trIdx < 36; trIdx++) {
      auto layer = std::make_shared<fl::Transformer>(
         ...
         1536, 384, 6144, 4, 920, 0.3, 0.3, false, false);
  ...
  std::vector<fl::Variable> forward(
    ...
    if (reshaping_factor != 1) {
      ...
      new_T += old_B - (new_T % old_B); // add this chunk so that new_T is divisible by old_B
    ...
    if (T_padded * old_B != new_T * new_B) {std::cout << "error, T_padded * old_B != new_T * new_B\n"; exit(0);}
    //////////////////////////
    af::array inputNotPaddedSize(1, old_B, 1, 1);
    for (int bIdx = 0; bIdx < old_B; bIdx++) { inputNotPaddedSize(0,bIdx,0,0) = old_T;} // TODO: use actual xSizes here
    ...
    return {ctc_head_out.as(input[0].type()), LID_head_out}; //.as(input[0].type())};
  }
 ...
 private:
  ...
  FL_SAVE_LOAD_WITH_BASE(fl::Container, convFrontend_, transformers_, linear_, LID_head_)
};
} // slimIPL

extern "C" fl::Module* createModule(int64_t nFeature, int64_t nLabel) {
  auto m = std::make_unique<slimIPL::myModel>(
		  nFeature, nLabel);
  return m.release();
}
