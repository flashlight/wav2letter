...
local function Mfcc (options)
   ...
   elseif type(options.N) ~= "number" then
     error("no number of cepstral coefficients (N) given")
   elseif type(options.L) ~= "number" then
     error("no liftering parameter (L) given")
   elseif type(options.dev) ~= "number" then
     error("no derivative context window size (dev) given")
   ...
   local nocep  = options.N
   --liftering parameter
   local L      = options.L
   --double delta (derivatives) context size
   local dev_s      = options.dev
   ...
   local post_flt = torch.Tensor()
   local fft_res  = torch.Tensor()
   local frames   = torch.Tensor()
   local post_dct = torch.Tensor()
   ...
   --Applies the DCT (symmetric inverse fft)
   --This could be done using a fft algorihtm, but I'm just doing
   --matrix multiplies.
   local dct = speech.Dct(nof, nocep)
   local cep = speech.Ceplifter(nocep, L, inplace)
   local dev = speech.Derivatives(dev_s, dev_s)
   ...
   return function (output_raw, input_raw)
      ...
      ----Apply tri filter
      post_flt = post_flt:type(input:type())
      post_flt:resize(frames:size(1), nof)
      triflt(post_flt, pow)
      --HTK wants to square this now and then apply the natural
      --logarithm (HTK uses that)
      post_flt:log():mul(2)

      post_dct = post_dct:type(input:type())
      post_dct:resize(frames:size(1), nocep)
      dct(post_dct, post_flt)
      cep(post_dct)
      output:resize(frames:size(1), 3*nocep)
      dev(output, post_dct)
