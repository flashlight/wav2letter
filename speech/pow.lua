...
local function Pow (options)
   ...
   return function (output_raw, input_raw)
      ...
      output:resize(frames:size(1), nfft)
      output[{{}, {1, fsize}}]:copy(frames)
      output[{{}, {fsize+1, nfft}}]:zero()
      --fft buffer tensor
      ...
      for i = 1, frames:size(1) do
         ...
         local res = fft(fft_res, output[i])
         output[i]:copy(res:norm(2, 2):squeeze())
      end
      ...
      --power spectrum
      output = output:narrow(fft_res:dim(), 1, K):cmax(mel_floor)
      return output
   end
end
