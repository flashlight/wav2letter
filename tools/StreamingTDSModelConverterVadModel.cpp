...
std::shared_ptr<streaming::LayerNorm3Axis>
convertLayerNorm(int featSz, const fl::Variable& wt, const fl::Variable& bs, int axis) {
  if (wt.elements() != 1 || bs.elements() != 1) {
    LOG(FATAL) << "Invalid params for layernorm.";
  }
  return std::make_shared<streaming::LayerNorm3Axis>(
      featSz, wt.scalar<float>(), bs.scalar<float>(), axis);
}
...
std::shared_ptr<streaming::TDSBlockWReorder> convertTDS(
    int channels,
    int kernelSz,
    int featSz,
    int rightPad,
  ...
  int axis = (lNormIncludeTime) ? 3 : 2;
  auto lnorm1 = convertLayerNorm(featSz * channels, params[2], params[3], axis);
  auto lnorm2 = convertLayerNorm(featSz * channels, params[8], params[9], axis);
  auto lin1 =
  ...
  return std::make_shared<streaming::TDSBlockWReorder>(
      conv1,
      lnorm1,
      lin1,
      lin2,
...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    if (layerType == "C2") {
      ...
      axis = columns.size() - 1;
      auto lyrNorm =
          convertLayerNorm(curFeatSz, params[paramIdx], params[paramIdx + 1], axis);
      ...
      auto linear = convertLinear(inDim, outDim,
                                  params[paramIdx],
                                  params[paramIdx + 1]);
      streamingModule->add(std::make_shared<streaming::Reorder>());
      ...
      auto stds = convertTDS(
          ...
          (columns.size() > 7) ? std::stoi(columns[7]) : 1);
  ...
  // Vad model uses 3 axis normalization. Fl normalizes whole chunk at once.
  // Inference framework in turn normalizes each frame separately.
  // This will lead to different results, so this check would be impossible.
  // To avoid this we pass data in finish method instead of run.
  // In this case whole chunk goes through network at once, so 3 axis
  // normalization works correctly.
  // auto outputState = streamingModule->run(inputState);
  auto outputState = streamingModule->finish(inputState);
