...
local function spellingremoveletterrepeat(spelling, maxr)
...
if opt.arpasrc:match('%.z$') or opt.arpasrc:match('%.gz$') then
   local f = io.popen(string.format('zcat %s', opt.arpasrc))
   txt = f:read('*all')
   f:close()
else
   local f = io.open(opt.arpasrc)
   txt = f:read('*all')
   f:close()
end
...
local farpa = io.open(opt.arpadst, 'w')
assert(farpa, string.format('cannot open <%s> for writing', opt.arpadst))
