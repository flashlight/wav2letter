...
netutils.create = argcheck{
   ...
   {name="pretrain", type="boolean"},
   {name="pre_model", type="nn.Sequential"},
   call =
      function(specs, gpu, nchannel, nclass, nspeakers, lsm, batchsize, wnorm, pretrain, pre_model)
         print('[Network spec]')
         print(specs)
        ...
        -- shoud sub 2 numbers from the line number
        local br_level = 0
        local kwdw = {}
        local osz = nchannel
        local osz_branch = -1
        ...
        -- add the original model up to the branch level
        for i=1, br_level do
          if torch.type(pre_model.modules[i]) ~= 'nn.Dropout' then
            model:add(pre_model.modules[i])
          end
        end
        -- add the branch from the original model
        local orig_branch = nn.Sequential()
        for i=(br_level + 1), #pre_model.modules do
          if torch.type(pre_model.modules[i]) ~= 'nn.Dropout' then
            orig_branch:add(pre_model.modules[i])
          end
        end
